...
export class CustomersComponent implements OnInit, OnDestroy {
  ...
  loadCustomers(): void {
    ...
    this.customerService
      .getAll(this.currentPage(), this.itemsPerPage(), this.searchTerm)
      .subscribe({
        next: (response) => {
  ...
  deleteCustomer(customer: Customer): void {
    if (customer.purchasesCount > 0) {
      // Aquí podrías también usar un modal de error
...
createCustomer(): void {
  ...
  this.customerService.create(customerData)
    .subscribe({
      next: (response: any) => { // 👈 CAMBIADO A any
        console.log('Respuesta del backend:', response);
        
        const newCustomer: Customer = {
          customerId: response.customerId || response.id || response._id || '',
          name: response.name || '',
          email: response.email || '',
          phone: response.phone || '',
          purchasesCount: response.purchasesCount ?? 0,
          isActive: response.isActive ?? true,
          createdAt: response.createdAt || new Date().toISOString(),
          updatedAt: response.updatedAt || new Date().toISOString()
        };

        console.log('Cliente normalizado:', newCustomer);

        if (this.searchTerm) {
          this.loadCustomers();
        } else {
          this.customers.update(customers => [newCustomer, ...customers.slice(0, 19)]);
          this.totalItems.update(total => total + 1);
        }
        this.closeModal();
      },
  ...
  toggleCustomerStatus(customer: Customer): void {
    ...
    this.customerService
      .toggleStatus(customer.customerId, newStatus)
      .subscribe({
